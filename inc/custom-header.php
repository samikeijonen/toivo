 ...
 *
 * @uses toivo_header_style()
 */
function toivo_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'toivo_custom_header_args', array(
		'default-image'          => '',
		'default-text-color'     => 'fff',
		'width'                  => 1920,
		'height'                 => 500,
		'flex-height'            => true,
		'wp-head-callback'       => 'toivo_header_style'
	) ) );

}
add_action( 'after_setup_theme', 'toivo_custom_header_setup', 15 );

if ( ! function_exists( 'toivo_header_style' ) ) :
/**
 * Styles the header image and text displayed on the blog
 *
 * @see toivo_custom_header_setup().
 */
function toivo_header_style() {
	...
	$min_width = absint( apply_filters( 'toivo_header_bg_show', 1 ) );
	
	/* Background arguments. */
	$background_arguments = esc_attr( apply_filters( 'toivo_header_bg_arguments', 'no-repeat 50% 50%' ) );
