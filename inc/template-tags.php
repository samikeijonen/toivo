...
if ( ! function_exists( 'toivo_comment_nav' ) ) :
/**
 * Display navigation to next/previous comments when applicable.
 *
 * @since 1.0.0
 */
function toivo_comment_nav( $class= '' ) {
	// Are there comments to navigate through?
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
	?>
	<nav class="navigation comment-navigation <?php echo esc_attr( $class ); ?>" role="navigation">
		<h2 class="screen-reader-text"><?php _e( 'Comment navigation', 'toivo' ); ?></h2>
		<div class="nav-links">
			<?php
				if ( $prev_link = get_previous_comments_link( __( '&larr; Older Comments', 'toivo' ) ) ) :
					printf( '<div class="nav-previous">%s</div>', $prev_link );
				endif;

				if ( $next_link = get_next_comments_link( __( 'Newer Comments &rarr;', 'toivo' ) ) ) :
...
if ( ! function_exists( 'toivo_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 *
 * @since 1.0.0
 */
function toivo_posted_on() {
	...
	printf( '<span class="entry-date"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s"' . hybrid_get_attr( 'entry-published' ) . '>%4$s</time></a></span>',
		_x( 'Posted on', 'Used before publish date.', 'toivo' ),
	...
	printf( '<span class="byline"><span class="entry-author" ' . hybrid_get_attr( 'entry-author' ) . '><span class="screen-reader-text">%1$s </span><a class="entry-author-link" href="%2$s" rel="author" itemprop="url"><span itemprop="name">%3$s</span></a></span></span>',
		_x( 'Author', 'Used before post author name.', 'toivo' ),
 ...
 * @return bool
 */
function toivo_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'toivo_categories' ) ) ) {
		...
		set_transient( 'toivo_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so toivo_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so toivo_categorized_blog should return false.
		return false;
	}
}
 ...
 * @return string
 */
function toivo_get_post_terms( $args = array() ) {
	...
	$defaults = array(
		...
		'sep'        => _x( ', ', 'taxonomy terms separator', 'toivo' )
	);
 ...
 * @return void
 */
function toivo_post_terms( $args = array() ) {
	echo toivo_get_post_terms( $args );
}

if ( ! function_exists( 'toivo_post_thumbnail' ) ) :
/**
...
/**
 * Flush out the transients used in toivo_categorized_blog.
 *
 * @since 1.0.0
 */
function toivo_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'toivo_categories' );
}
add_action( 'edit_category', 'toivo_category_transient_flusher' );
add_action( 'save_post',     'toivo_category_transient_flusher' );
