...
function toivo_setup() {
	...
	*/
	 ...
	 * to change 'toivo' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'toivo', get_template_directory() . '/languages' );
	...
	register_nav_menus( array( 
		'primary' => _x( 'Primary', 'nav menu location', 'toivo' ),
		'top'     => _x( 'Top', 'nav menu location', 'toivo' ),
		'social'  => _x( 'Social', 'nav menu location', 'toivo' )
	) );
	...
	/* Add editor styles. */
	add_editor_style( toivo_get_editor_styles() );
...
add_action( 'after_setup_theme', 'toivo_setup' );
 ...
 */
function toivo_widgets_init() {

	$sidebar_primary_args = array(
		'id'            => 'primary',
		'name'          => _x( 'Primary', 'sidebar', 'toivo' ),
		'description'   => __( 'The main sidebar. It is displayed on the right or left side of the page.', 'toivo' ),
	...
	$sidebar_subsidiary_args = array(
		'id'            => 'subsidiary',
		'name'          => _x( 'Subsidiary', 'sidebar', 'toivo' ),
		'description'   => __( 'A sidebar located in the footer of the site.', 'toivo' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>'
	...
	$sidebar_front_page_args = apply_filters( 'toivo_sidebar_front_page_args', array(
		'id'            => 'front-page',
		'name'          => _x( 'Front Page', 'sidebar', 'toivo' ),
		'description'   => __( 'A sidebar located in the Front Page Template.', 'toivo' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>'
...
add_action( 'widgets_init', 'toivo_widgets_init' );

/**
 * Return the Google font stylesheet URL
 */
function toivo_fonts_url() {
	 ...
	 */
	$lato = _x( 'on', 'Lato font: on or off', 'toivo' );

	/* Translators: If there are characters in your language that are not
	 * supported by Raleway, translate this to 'off'. Do not translate
	 * into your own language.
	 */
	$raleway = _x( 'on', 'Raleway font: on or off', 'toivo' );
 ...
 */
function toivo_scripts() {

	/* Enqueue fonts. */
	wp_enqueue_style( 'toivo-fonts', toivo_fonts_url(), array(), null );
	...
	if ( is_child_theme() ) {
		wp_enqueue_style( 'toivo-parent-style', trailingslashit( get_template_directory_uri() ) . 'style.css', array(), TOIVO_VERSION );
	}
	
	/* Enqueue active theme styles. */
	wp_enqueue_style( 'toivo-style', get_stylesheet_uri() );
	
	/* Enqueue responsive navigation if one of the menus is in use. */
	if ( has_nav_menu( 'primary' ) || has_nav_menu( 'top' ) || has_nav_menu( 'social' ) ) {
		wp_enqueue_script( 'toivo-navigation', get_template_directory_uri() . '/js/responsive-nav.js', array(), TOIVO_VERSION, true );
	
		/* Enqueue responsive navigation settings. */
		wp_enqueue_script( 'toivo-settings', trailingslashit( get_template_directory_uri() ) . 'js/settings.js', array( 'toivo-navigation' ), TOIVO_VERSION, true );
	}
	
	/* Enqueue functions. */
	wp_enqueue_script( 'toivo-script', get_template_directory_uri() . '/js/functions.js', array(), TOIVO_VERSION, true );
...
add_action( 'wp_enqueue_scripts', 'toivo_scripts' );
...
add_action( 'template_redirect', 'toivo_one_column' );
 ...
 * @return string
 */
function toivo_theme_layout_one_column( $layout ) {
	return '1c';
}
 ...
 * @since  1.0.0
 */
function toivo_nav_description( $item_output, $item, $depth, $args ) {
	if ( 'primary' == $args->theme_location && $item->description ) {
		$extra_class = ( 0 === $depth ) ? ' top-depth' : '';
		$item_output = str_replace( $args->link_after . '</a>', '<span class="menu-item-description' . $extra_class . '"><span class="menu-item-description-mark">' . ' ' . _x( '&ndash;' , 'dash before menu item description', 'toivo' ) . '</span> ' . $item->description . '</span>' . $args->link_after . '</a>', $item_output );
	}

	return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'toivo_nav_description', 10, 4 );
 ...
 * @since  1.0.0
 */
function toivo_infinity_after_aside( $content ) {
...
add_filter( 'the_content', 'toivo_infinity_after_aside', 9 ); // run before wpautop
 ...
 * @return  array
 */
function toivo_get_editor_styles() {
 ...
 * @since 1.0.0
 */
function toivo_comment_callback( $comment, $args, $depth ) {
